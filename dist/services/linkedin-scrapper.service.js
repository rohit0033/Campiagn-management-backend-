...
const proxy_manager_service_1 = require("./proxy-manager.service");
// --- Helper Functions ---
const userAgents = [
...
const statusLog = (section, message, sessionId = "") => {
...
class LinkedInProfileScraper {
    ...
    async setup(usePersistentSession = false) {
        ...
        try {
            ...
            // Get a proxy
            const proxy = proxy_manager_service_1.proxyManager.getNextProxy();
            ...
            this._browser = await puppeteer_extra_1.default.launch({
                headless: true,
                userDataDir,
                args: [
                    ...proxyArgs,
    ...
    async scrapeProfile(profileUrl) {
        ...
        try {
            ...
            if (page.url().includes("/login") || page.url().includes("/authwall")) {
                statusLog(logSection, "Redirected to login/auth wall. Cookie might be invalid or expired.", sessionId);
                throw new Error("Authentication required. Check your li_at cookie.");
            }
            const currentUrlBeforeScroll = page.url();
            statusLog(logSection, "Scrolling page to load dynamic content...", sessionId);
            ...
            try {
                const seeMoreButtonSelector = "#about + div .inline-show-more-text__button--light";
                if ((await page.$(seeMoreButtonSelector)) !== null) {
                    statusLog(logSection, `Found "see more" button [${seeMoreButtonSelector}]. Clicking...`, sessionId);
                    await page.click(seeMoreButtonSelector);
                    await delay(800, 2000);
                    statusLog(logSection, 'Clicked "see more" for About.', sessionId);
                }
                else {
                    statusLog(logSection, `About section "see more" button [${seeMoreButtonSelector}] not found or section already expanded.`, sessionId);
                }
            }
            catch (err) {
                statusLog(logSection, sessionId);
            }
            await delay(1200, 3500);
            // --- Extract Data ---
            try {
                statusLog(logSection, "Extracting profile data using updated selectors...", sessionId);
                const profileData = await page.evaluate(() => {
                    const getText = (selector) => { var _a, _b; return ((_b = (_a = document.querySelector(selector)) === null || _a === void 0 ? void 0 : _a.textContent) === null || _b === void 0 ? void 0 : _b.trim()) || null; };
                    const selectors = {
                        name: "h1",
            ...
            if (page) {
                try {
                    ...
                    statusLog(logSection, `Error closing page: ${closeErr.message}`, sessionId);
                }
            }
        }
    }
...
async function scrapeLinkedInProfile(linkedin_url, sessionCookieValue, usePersistentSession = false) {
...
async function extractPublicProfileData(page) {
    return await page.evaluate(() => {
        var _a, _b, _c, _d;
        const name = ((_b = (_a = document.querySelector('h1')) === null || _a === void 0 ? void 0 : _a.textContent) === null || _b === void 0 ? void 0 : _b.trim()) || null;
        const description = ((_d = (_c = document.querySelector('.top-card-layout__headline')) === null || _c === void 0 ? void 0 : _c.textContent) === null || _d === void 0 ? void 0 : _d.trim()) || null;
        return {
            name,
            description,
            jobTitle: null,
...
async function runTest() {
    const LINKEDIN_SESSION_COOKIE = "YOUR_LI_AT_COOKIE_VALUE_GOES_HERE";
    const PROFILE_URL = "https://www.linkedin.com/in/kunikamalhotra/";
    if (LINKEDIN_SESSION_COOKIE !== "YOUR_LI_AT_COOKIE_VALUE_GOES_HERE") {
        try {
            const profileDataWithCookie = await scrapeLinkedInProfile(PROFILE_URL, LINKEDIN_SESSION_COOKIE);
    ...
    try {
        const profileDataPersistent = await scrapeLinkedInProfile(PROFILE_URL, undefined, true);
